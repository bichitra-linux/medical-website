...
export default function StaffsPage() {
  const [activeStaffs, setActiveStaffs] = useState<Doctor[]>([]);
  const [formerStaffs, setFormerStaffs] = useState<Doctor[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const { activeStaffs, formerStaffs } = await res.json();
        setActiveStaffs(activeStaffs);
        setFormerStaffs(formerStaffs);
      } catch (e: any) {
        setError(e.message);
      } finally {
        setLoading(false);
  ...
  const specialties = ["all", ...Array.from(new Set(activeStaffs.map((d) => d.specialty)))];

  const filtered = activeStaffs
    .filter((d) => {
      const matchesSearch =
        debouncedSearch === "" ||
  ...
  return (
    <>
      ...
      <section className="bg-gradient-to-r from-blue-50 to-blue-100 py-16 md:py-20 text-center">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-800">Our Staffs</h1>
        ...
        <div className="container mx-auto px-4">
          <Tabs defaultValue="active">
            ...
            <TabsContent value="active">
              {filtered.length === 0 ? (
                <div className="text-center py-20 text-gray-500">No Staffs found.</div>
              ) : (
                    ...
                    >
                      ...
                      <CardHeader className="pb-2 px-4">
                        <CardTitle className="text-xl text-gray-800 hover:text-blue-600 transition-colors">
                          <Link href={`/Staffs/${d.id}`}>{d.name}</Link>
                        </CardTitle>
